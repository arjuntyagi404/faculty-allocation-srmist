...
def _rebuild_faculty_for_nullable_import_fields(db, columns):
    ...
    db.session.execute(db.text(
        ...
        CREATE TABLE faculty_new (
            ...
            special_role VARCHAR(150) DEFAULT 'None'
        )
        """
    ))
    db.session.execute(db.text(
...
def migrate_schema(db):
    ...
    subject_columns = {
        row[1]
        for row in db.session.execute(
            db.text("PRAGMA table_info(subjects)")
        ).fetchall()
    }
    for column_name, column_type in (
        ...
        if subject_columns and column_name not in subject_columns:
            db.session.execute(db.text(
                f"ALTER TABLE subjects ADD COLUMN {column_name} {column_type}"
            ))

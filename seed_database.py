...
def _subject_row(row):
    return {
        ...
        "semester_type": _value(row, "semester_type"),
...
def import_subject_master():
    rows = _read_csv("Subject_Master.csv", ("subject_code",), _subject_row)
    merged = {}
    duplicate_codes = set()
    for row in rows:
        ...
        current = merged[code]
        if sum(value is not None for value in row.values()) > sum(
        ...
        for field, value in row.items():
            if current.get(field) is None and value is not None:
                current[field] = value
    if duplicate_codes:
        print("Duplicate subject codes resolved: " + ", ".join(sorted(duplicate_codes)))
...
def _validation_errors(faculties, subjects, theory, labs):
    ...
    for row_number, row in enumerate(faculties, 2):
        ...
        if any(
            role not in SPECIAL_ROLES
            for role in (row.get("special_role") or "None").split(",")
        ):
